'''
Since our initial experiments last year and for patenting we have slightly improved the network. Like instead o$
from 0.1 to 0.9 it now ranges from 1 to 9. We also found that modern CUDA libraries have been specially designe$
and by design larger kernels will be slower. Thus we additionally discourage use of large kernels. Further 
'Learning Raw Image Denoising with Bayer Pattern Unification and Bayer Preserving Augmentation' suggests specia$
augmentations for Bayer pattern. Use them for better restoration. For better performance use different learning$
conv. weights and bias (https://github.com/cydonia999/Learning_to_See_in_the_Dark_PyTorch/blob/470a6a8e9c6367d8$
...
opt['atWhichReduce'] = [350000] # Reduce learning rate at these iterations.
opt['batch_size'] = 8
opt['atWhichSave'] = [2,100002,150002,200002,250002,300002,350002,400002,450002,500002] # ,550000, 600000,65000$
...
metric_average_file = 'metric_average.txt' # Average metrics will be saved here. Please note these are only for$
test_amplification_file = 'test_amplification.txt' # Intermediate details for the test images, such as estimate$
train_amplification_file = 'train_amplification.txt' # Intermediate details for the train images, such as estim$
...
from vainF_ssim import MS_SSIM, SSIM

os.environ["CUDA_DEVICE_ORDER"]="PCI_BUS_ID"   
os.environ["CUDA_VISIBLE_DEVICES"]="0" # You would probably like to change it to 0 or some other integer depend$
...
gt_files = []
for x in test_files:
    gt_files = gt_files+ glob.glob('../../Sony_small/Sony/long/*'+x[-17:-12]+'*.ARW')#NOTICE THIS IS THE SAME A$
